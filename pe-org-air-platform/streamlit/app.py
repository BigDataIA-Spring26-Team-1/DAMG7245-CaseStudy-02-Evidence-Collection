...
with info_col:
    ...
    st.write("Health routes are not prefixed. All other routes use the API prefix.")
        ...
        if not status_assessment_id.strip():
            ...
            try:
                url = _api_url(
                    api_base,
                    api_prefix,
                    f"/assessments/{status_assessment_id.strip()}/status",
                )
            ...
            if not upsert_assessment_id.strip():
                ...
                try:
                    url = _api_url(
                        api_base,
                        api_prefix,
                        f"/assessments/{upsert_assessment_id.strip()}/scores",
                    )
                    data = _request_json("POST", url, json=payload, timeout=timeout)
            ...
            if not raw_assessment_id.strip():
                ...
                try:
                    url = _api_url(
                        api_base,
                        api_prefix,
                        f"/assessments/{raw_assessment_id.strip()}/scores",
                    )
                    data = _request_json("POST", url, json=payload, timeout=timeout)
                    st.success("Score upserted")
                    st.json(data)
