...
class DocumentStatus(str, Enum):
   ...
   INDEXED = "indexed"
   FAILED = "failed"
   PROCESSED = "processed"  # legacy/default
...
class EvidenceStore:
   """
   Snowflake persistence for CS2 evidence collection.
   Tables expected (per schema.sql):
     - documents
     - document_chunks
   """
   def __init__(self) -> None:
       self.conn = get_snowflake_connection()
       # make inserts/updates visible immediately, avoids forgetting conn.commit()
       try:
   ...
   def insert_chunks_bulk(self, chunks: Sequence[ChunkRow]) -> None:
       ...
       try:
           ...
           cur.close()
   # -------------------------
   # Status updater (Document Registry requirement)
   # -------------------------
   def update_document_status(
       ...
       """
       Updates documents.status and optionally error_message.
       Also stamps processed_at when a document reaches terminal states (indexed/failed).
       """
       q = """
       UPDATE documents
          SET status=%s,
   ...
   def insert_failed_stub(
       ...
       """
       Inserts a minimal 'failed' row when the pipeline errors before the document row is inserted.
       """
       q = """
       INSERT INTO documents (
         id, company_id, ticker, filing_type, filing_date,
       ...
       try:
           cur.execute(
               ...
               (
                   ...
                   local_path,
                   content_hash,
                   0,  # word_count unknown
                   0,  # chunk_count unknown
   ...
   def get_document(self, document_id: str) -> dict[str, Any] | None:
       q = """
       SELECT id, company_id, ticker, filing_type, filing_date, source_url, local_path,
              content_hash, word_count, chunk_count, status, error_message, created_at, processed_at
       FROM documents
       WHERE id = %s
       """
       cur = self.conn.cursor()
